...
import { sendBackingTrackReady, sendSessionState, sendSoloAnalysisReady, sendSoloistMidi, } from '../ws/sessionHandler.js';
...
sessionsRouter.post('/sessions/:id/stop', (req, res) => {
    ...
    try {
        ...
        if (soloistNotes.length > 0) {
            const analysis = analysisService.analyze(id, session.config.chordChart, soloistNotes, session.currentChorus, 'soloist');
            sendSoloAnalysisReady(id, analysis);
        }
        else if (playerNotes.length > 0) {
            const analysis = analysisService.analyze(id, session.config.chordChart, playerNotes, session.currentChorus, 'player');
            sendSoloAnalysisReady(id, analysis);
        }
        sendSessionState(id, session);
        res.json(session);
