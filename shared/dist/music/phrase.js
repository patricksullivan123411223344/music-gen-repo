...
export function splitPhrases(rows, phraseGapBeats = DEFAULT_PHRASE_GAP) {
    if (rows.length === 0)
        return [];
    const sorted = [...rows].sort((a, b) => a.beat - b.beat);
    const phrases = [[sorted[0]]];
...
export function summarizePhrase(rows) {
    if (rows.length === 0) {
        return {
            density: 'sparse',
            contour: 'level',
    ...
    const sorted = [...rows].sort((a, b) => a.beat - b.beat);
    const span = Math.max(0.25, sorted[sorted.length - 1].beat +
        sorted[sorted.length - 1].duration -
        sorted[0].beat);
    const densityVal = sorted.length / span;
    const density = densityVal < 0.55 ? 'sparse' : densityVal < 1.2 ? 'moderate' : 'active';
    ...
    const tensionRatio = sorted.filter((r) => r.category === 'tension_tone' || r.category === 'outside').length / sorted.length;
    const tension = tensionRatio === 0
        ? 'inside'
        : tensionRatio < 0.25
            ? 'light'
            : tensionRatio < 0.45
...
export function choosePhraseForResponse(rows, minNotes = 3, phraseGapBeats = DEFAULT_PHRASE_GAP) {
    const phrases = splitPhrases(rows, phraseGapBeats);
    for (let i = phrases.length - 1; i >= 0; i--) {
        if (phrases[i].length >= minNotes)
            return phrases[i];
    }
