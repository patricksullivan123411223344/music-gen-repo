...
import { musicDefaults } from './settings.js';
function clamp(n, lo, hi) {
    return Math.max(lo, Math.min(hi, n));
}
function extractMotif(rows, maxNotes = 8) {
    ...
    for (let i = 0; i < slice.length; i++) {
        ...
        if (i > 0)
            intervals.push(slice[i].midi - slice[i - 1].midi);
    }
    return {
        intervals,
...
function stablePcs(quality, rootPc) {
    const tones = chordToneIntervals(quality);
    return tones.map((iv) => (rootPc + iv) % 12);
}
function snapMidi(midi, pcs, prev, maxLeap) {
    let best = midi;
...
export function wantsChorusTrade(mode, inputSpan, formBeats, threshold = musicDefaults.chorusTradeThreshold) {
    if (mode === 'chorus')
        return true;
    if (mode === 'phrase')
        return false;
...
export function buildPhraseTradeResponse(chart, inputNotes, memory, tradeMode = musicDefaults.tradeMode) {
    ...
    const inputSpan = Math.max(...inputNotes.map((n) => n.startBeat + n.durationBeats)) -
    ...
    const startBeat = Math.max(...inputNotes.map((n) => n.startBeat + n.durationBeats)) +
        (chorus ? 0 : musicDefaults.audioResponseGapBeats);
    const intervals = memory?.motifIntervals.length ?
        memory.motifIntervals
        : motif.inverted.length > 0 ?
            motif.inverted
            : [2, -1, 2, -2];
    const durations = motif.durations.length > 0 ? motif.durations : [0.5, 0.5, 0.5, 1];
    const seed = memory?.registerCenter ??
        Math.round(motif.midis.reduce((a, b) => a + b, 0) / Math.max(1, motif.midis.length));
    const notes = [];
    let t = startBeat;
    let midi = seed;
    let i = 0;
    while (t < startBeat + answerBeats - musicDefaults.soloResolutionBeats) {
        const section = Math.floor(((t - startBeat) / answerBeats) * 4);
    ...
    const pcs = stablePcs(at.chord.quality, at.chord.rootPc);
    const land = snapMidi(memory?.lastResolutionMidi ?? midi, pcs, midi, musicDefaults.soloMaxLeap);
    notes.push({
        pitch: land,
        velocity: 86,
        startBeat: resStart,
