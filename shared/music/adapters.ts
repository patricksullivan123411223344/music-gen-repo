...
import { summarizePhrase, type PhraseSummary } from './phrase.js';

export function categoryToNoteFunction(category: NoteCategory): NoteFunction {
  if (category === 'chord_tone') return 'chord_tone';
  if (category === 'color_tone') return 'color_tone';
  return 'tension_tone';
}

export function analysisRowsToAnalyzedNotes(rows: AnalysisRow[]): AnalyzedNote[] {
  return rows.map((row) => ({
    pitch: row.midi,
