...
function voicingIntervals(chord: Chord): number[] {
  const base = chordToneIntervals(chord.quality);
  // drop root sometimes for jazz shells — keep 3 + 7 + color
  if (base.length >= 4) return [base[1], base[3], base[0] + 12];
  return base.map((iv, i) => (i === 0 ? iv : iv));
}

function chordVoicing(chord: Chord, centerMidi = 60): number[] {
  const ivs = voicingIntervals(chord);
  return ivs.map((iv) => {
    let m = chord.rootPc + 48 + iv;
    while (m < centerMidi - 8) m += 12;
    while (m > centerMidi + 10) m -= 12;
    return m;
  });
}

function drumHitsForStyle(
  style: BackingStyle,
  totalBeats: number,
  energy: BandEnergy,
): MidiNoteEvent[] {
...
function buildBass(
  ...
  const notes: MidiNoteEvent[] = [];
  for (let beat = 0; beat < totalBeats; beat++) {
    const at = events.find(
      (e) => beat >= e.start && beat < e.start + e.duration,
    ...
    if (walking) {
      const step = beat % 4;
      const pitch =
        step === 0 ? root : step === 1 ? fifth : step === 2 ? root + 12 : root + 1;
      notes.push({
        pitch,
        velocity: 90,
        startBeat: beat,
...
function buildComping(
  ...
  for (const ev of events) {
    ...
    if (style === 'bebop') {
      ...
      if (ev.duration >= 3) hitStarts.push(ev.start + 2);
    }
    for (const start of hitStarts) {
      if (start >= ev.start + ev.duration) continue;
...
export function buildBackingParts(
  ...
  const looped: ChordEvent[] = [];
  if (events.length > 0) {
    const formLen =
      events[events.length - 1].start + events[events.length - 1].duration;
    for (const ev of events) {
      looped.push(ev);
...
export function chartLengthBeatsFromChart(chart: ChordChart): number {
